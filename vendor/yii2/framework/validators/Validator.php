...
class Validator extends Component
{
    ...
    public function validateAttributes($model, $attributes = null)
    {
        if (is_array($attributes)) {
            $attributes = array_intersect($this->attributes, $attributes);//找出需要被验证的属性
        } else {
            $attributes = $this->attributes;
        }
        foreach ($attributes as $attribute) {
            $skip = $this->skipOnError && $model->hasErrors($attribute)
                || $this->skipOnEmpty && $this->isEmpty($model->$attribute);//判断如果此属性有一个验证错误或者属性值为空时候是否跳过验证
            if (!$skip) {
                if ($this->when === null || call_user_func($this->when, $model, $attribute)) {//验证一个具体属性之前的回掉函数
